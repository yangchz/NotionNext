...
const umamiCaller = {
  fetch: function (callback) {
    const baseUrl = BLOG.UMAMI_SCRIPT_URL?.split("/").slice(0, -1).join("/");
    const websiteId = BLOG.UMAMI_WEBSITE_ID;
    const token = BLOG.UMAMI_TOKEN;
    const data = {
        headers: {
    ...
    if (BLOG.UMAMI_ENABLED && baseUrl && websiteId && token) {
